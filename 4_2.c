 ...
 * @param cnt - Промежуточный минимальный по модулю положительный элемент массива
...
/**
 * @brief Функция, которая считает новый массив по правилу: элементы с 3-его по 12-ый находятся по формуле А(i)=(-1)*(D(i)*D(i)), остальные по формуле А(i)=D(i)-1
 * @return Этот массив
 */
...
int main(void)
{
	...
	printf("новый массив по правилу: элементы с 3-его по 12-ый находятся по формуле А(i)=(-1)*(D(i)*D(i)), остальные по формуле А(i)=D(i)-1 \n");
